...
def proc_and_save_unsup_data(
  ...
  if sub_set == "unsup_fix":
    token_num = float(aug_ops.split("-")[1])
    ori_examples = processor.get_weak_examples(raw_data_dir, token_num)
    tf.logging.info("getting augmented examples")
    aug_examples = processor.get_strong_examples(raw_data_dir, token_num)
