...
def create_model(
  ...
  with tf.variable_scope("sup_loss"):
    ...
    sup_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels,
                                                                logits=clas_logits[:sup_batch_size])
  ...
  if is_training and unsup_ratio > 0:
    with tf.variable_scope("unsup_loss"):
      ...
      logits_weak = clas_logits[ori_start : ori_end]
      logits_strong = clas_logits[aug_start : aug_end]
      pseudo_labels = tf.stop_gradient(tf.nn.softmax(logits_weak, axis=-1))
      unsup_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tf.argmax(pseudo_labels, axis=-1),
                                                                logits=logits_strong)
      confidence = 0.95      #       Confidence threshold                                              
      unsup_loss_mask = tf.to_float(tf.reduce_max(pseudo_labels, axis=-1) >= confidence)
      unsup_loss = tf.reduce_mean(unsup_loss * unsup_loss_mask)
...
def model_fn_builder(
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      # lrate = tf.clip_by_value(tf.to_float(global_step) / num_train_steps, 0, 1)
      # curr_lr = learning_rate*tf.cos(lrate * (7 * np.pi) / (2 * 8))
      # optimizer = tf.train.MomentumOptimizer(curr_lr, 0.9, use_nesterov=True)
      # if use_tpu:
